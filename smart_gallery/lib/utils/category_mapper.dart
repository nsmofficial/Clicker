...
class CategoryMapper {
  ...
  /// Check if label matches keyword using word-boundary matching.
  /// Avoids false positives like "calendar" matching "cat".
  static bool _matchesKeyword(String labelText, String keyword) {
    // Exact match
    if (labelText == keyword) return true;
    // Multi-word keyword is contained in label
    if (keyword.contains(' ') && labelText.contains(keyword)) return true;
    // Single-word keyword: check word boundaries
    final words = labelText.split(RegExp(r'[\s,_\-]+'));
    ...
    for (final label in labels) {
      ...
      for (final entry in _categoryKeywords.entries) {
        for (final keyword in entry.value) {
          if (_matchesKeyword(labelText, keyword)) {
